  ...
  export default {
    ...
    methods: {
      async setupMiniSearch(items) {
        ...
        if (this.$seneca && this.$seneca.__vxgMiniSearchSeeded) {
          this.miniSearchSeeded = true
          return
        }
        if (this.miniSearchSeeded) {
          return
        }
        this.miniSearchSeeded = true
        if (this.$seneca) {
          this.$seneca.__vxgMiniSearchSeeded = true
        }
        ...
        for(const item of items) {
          try {
            ...
            // Do not fail UI boot for indexing issues (duplicates/stale index).
            if (typeof window !== 'undefined' && window.devLog) {
              window.devLog('mini-search add skipped:', error && (error.message || error))
            }
          }
        }
      },
